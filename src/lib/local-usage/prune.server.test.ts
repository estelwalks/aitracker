...
import {
  ...
  directorySize,
  isControlledTrustToolsDirectory,
...
test("retention only removes expired files from a marked TrustTools cache", async () => {
...
test("refuses an unmarked external directory even when it contains a cache folder", async () => {
  ...
  assert.equal(await isControlledTrustToolsDirectory(externalRoot), false);
  const cleanup = await clearRegenerableCache(externalRoot);
  assert.equal(cleanup.skipped, true);
  assert.match(cleanup.reason ?? "", /未经 TrustTools 验证/);
