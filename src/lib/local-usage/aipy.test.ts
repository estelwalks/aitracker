 ...
 * directory. The adapter query must prefer the workspace's workdir so AiPy
 * usage can be grouped by project instead of collapsing to per-task scratch
 * directories.
 */
async function withFixture<T>(
  ...
  try {
    db.exec(`
      ...
      CREATE TABLE task (id TEXT, model TEXT, workdir TEXT, workspace_id TEXT);
...
test("AiPy adapter query prefers workspace.workdir over per-task workdir", async () => {
  await withFixture(async (db) => {
    db.exec(`
      INSERT INTO workspace (id, name, workdir) VALUES ('ws1', 'Real Project', '/work/real-project');
      INSERT INTO task (id, model, workdir, workspace_id) VALUES
        ('t1', 'auto', '/work/aipywork/1', 'ws1'),
        ('t2', 'auto', '/work/aipywork/2', NULL);
    ...
    assert.equal(bySession.get("t2")?.project, "/work/aipywork/2");
  });
});
