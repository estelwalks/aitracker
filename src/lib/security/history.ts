 ...
 * - Electron：`window.trustToolsDesktop.getPreferences` / `setPreference`
...
interface TrustToolsDesktopApiLike {
  getPreferences(): Promise<Record<string, unknown>>;
  setPreference(key: string, value: unknown): Promise<void>;
}

function getDesktopApi(): TrustToolsDesktopApiLike | undefined {
  if (typeof window === "undefined") return undefined;
  return (
    window as {
      trustToolsDesktop?: TrustToolsDesktopApiLike;
    }
  ).trustToolsDesktop;
}
