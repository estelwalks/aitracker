...
import { catalogs, getMessage, type Translations } from "./messages.ts";
...
test("getMessage: 无参数消息按当前语言解析", () => {
  assert.equal(getMessage(catalogs["zh-CN"], "nav.dashboard"), "首页");
  assert.equal(getMessage(catalogs["en-US"], "nav.dashboard"), "Dashboard");
  assert.equal(getMessage(catalogs["ja-JP"], "nav.dashboard"), "ホーム");
  assert.equal(getMessage(catalogs["ko-KR"], "nav.dashboard"), "홈");
});
...
test("getMessage: 复数 {one, other} 按 count 选择(en-US)", () => {
