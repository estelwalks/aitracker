  ...
  const filtered = useMemo(() => {
    ...
    return summary.entries.filter(
      (entry) =>
        ...
        (!kw || entry.nameZh.toLowerCase().includes(kw)),
    );
          ...
          {STATUS_FILTERS.map((filter) => {
            ...
            return (
              <button
                ...
                className={`tt-num inline-flex items-center gap-1.5 rounded-sm border px-2.5 py-1 text-[12px] transition-colors ${active ? "border-primary bg-primary/10 text-primary" : "border-border bg-surface text-muted-foreground hover:text-foreground"}`}
              >
                {t(filter.labelKey)}
...
function SourceRow({ entry }: { entry: SourcesQueryEntry }) {
  const { t, format } = useI18n();
