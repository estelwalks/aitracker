...
test("model legacy import is repeatable, preserves active profile and existing secret", async (t) => {
  const host = fixture(t);
  const repository = createSqliteModelProfileRepository({
    database: host,
    secretCodec: codec,
  ...
  const source = {
    ...
    activeProfileId: "m-legacy",
  };
  assert.deepEqual(await repository.importLegacy(source), {
    profilesImported: 1,
    secretsSkipped: 0,
  });
  assert.deepEqual(await repository.importLegacy(source), {
  ...
  assert.equal((await repository.getActiveView())?.id, "m-legacy");
  assert.equal(
    (await repository.getProfileForExecution("m-legacy"))?.apiKey,
    "legacy-secret",
  );
  assert.deepEqual(await repository.remove("m-legacy"), { ok: true });
  assert.equal(
    host.prepare("SELECT COUNT(*) AS n FROM secure_secrets").get()?.n,
    0n,
  );
