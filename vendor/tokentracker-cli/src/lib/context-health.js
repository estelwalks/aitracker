...
function readMetadata(filePath, kind, root) {
  try {
    ...
    return {
      kind,
      name: path.relative(root, filePath) || path.basename(filePath),
...
function countMcpServers(home, cwd) {
  ...
  try {
    ...
    for (const match of toml.matchAll(/^\s*\[mcp_servers\.([^\]]+)\]/gm))
      names.add(match[1]);
  } catch {
    /* absent */
  }
...
function computeContextHealth({
  ...
  const instructionTokens = files
    .filter((row) => row.kind === "instruction")
    .reduce((sum, row) => sum + row.estimated_tokens, 0);
  const skillTokens = files
    .filter((row) => row.kind === "skill")
    .reduce((sum, row) => sum + row.estimated_tokens, 0);
  const mcpServers = countMcpServers(home, cwd);
  const perToolSchemaTokens = Math.max(
    0,
    Number(env.TOKENTRACKER_MCP_TOOL_SCHEMA_TOKENS || 400),
  );
  const assumedToolsPerServer = Math.max(
    1,
    Number(env.TOKENTRACKER_MCP_TOOLS_PER_SERVER || 5),
  );
  const mcpSchemaTokens =
    mcpServers * perToolSchemaTokens * assumedToolsPerServer;
  const total = instructionTokens + skillTokens + mcpSchemaTokens;
  const severity =
    total >= 50_000 ? "high" : total >= 20_000 ? "medium" : "low";
  const result = {
    generated_at: new Date().toISOString(),
    estimated_fixed_tokens: total,
    severity,
    breakdown: {
      instruction_files: files.filter((row) => row.kind === "instruction")
        .length,
    ...
    provenance: {
      ...
      mcp_assumption: {
        tools_per_server: assumedToolsPerServer,
        tokens_per_tool_schema: perToolSchemaTokens,
      },
