...
function sessionsFingerprint(sessions, maxAgeDays) {
  ...
  const rows = sessions.map(
    (session) =>
      `${session.project_ref}\0${session.session_hash}\0${session.started_at}\0${session.ended_at}`,
  );
...
function attributableSessions(sessions, cutoff, home) {
  ...
  for (const session of sessions || []) {
    if (!session?.project_ref || !session.started_at || !session.ended_at)
      continue;
...
function gitAttributionDisabled() {
  return ["1", "true"].includes(
    String(
      process.env.TOKENTRACKER_DISABLE_GIT_ATTRIBUTION || "",
    ).toLowerCase(),
  );
}
...
function protectedProbeAllowed() {
  return ["1", "true"].includes(
    String(
      process.env.TOKENTRACKER_GIT_ATTRIBUTION_PROTECTED_DIRS || "",
    ).toLowerCase(),
  );
}
...
async function buildGitOutcomesInternal(
  sessions,
  { home = os.homedir(), force = false, maxAgeDays = 90 } = {},
) {
  ...
  if (!force) {
    try {
      ...
      /* first run, or a v1 sidecar written before this gate existed */
    }
  }
  ...
  for (const [root, repoSessions] of [...groups.entries()].sort(([a], [b]) =>
    a.localeCompare(b),
  )) {
    signatureHash.update(
      `${root}\0${await runGit(root, ["for-each-ref", "--format=%(objectname)"])}\n`,
    );
    for (const session of repoSessions)
      signatureHash.update(`${session.session_hash}\0${session.ended_at}\n`);
  }
  ...
  if (!force) {
    try {
      ...
      if (meta.signature === signature) {
        await writeAtomic(
          metaPath,
          `${JSON.stringify({ ...meta, version: 2, session_signature: sessionSignature, generated_at: new Date().toISOString() })}\n`,
        );
        return readJsonl(outputPath);
      }
    } catch {
  ...
  for (const [root, repoSessions] of groups) {
    const start = repoSessions.reduce(
      (min, row) => (!min || row.started_at < min ? row.started_at : min),
      "",
    );
    const endMs =
      Math.max(...repoSessions.map((row) => Date.parse(row.ended_at))) +
      60 * 60 * 1000;
    const commits = await commitsForWindow(
      root,
      start,
