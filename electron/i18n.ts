...
export const electronMessages: Record<DesktopLocale, ElectronMessages> = {
  "zh-CN": {
    tray: { tooltip: "TrustTools" },
    menu: { open: "打开 TrustTools", autoLaunch: "开机自动启动", quit: "退出" },
    dialog: {
      closeHint: {
  ...
  "en-US": {
    ...
    dialog: {
      closeHint: {
        message:
          "TrustTools will keep running in the menu bar — reopen it from the tray icon",
        ok: "Got it",
      },
      dataIncompat: {
  ...
  "ja-JP": {
    ...
    dialog: {
      closeHint: {
        message:
          "TrustTools はメニューバーで実行を続けます。トレイアイコンから再度開けます",
        ok: "了解しました",
      },
      dataIncompat: {
        title: "データバージョン非互換",
  ...
  "ko-KR": {
    ...
    dialog: {
      closeHint: {
        message:
          "TrustTools는 메뉴 막대에서 계속 실행됩니다. 트레이 아이콘으로 다시 열 수 있습니다",
        ok: "확인",
      },
      dataIncompat: {
        title: "데이터 버전 비호환",
