...
<html lang="${locale}">
  ...
  <style>
    :root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;background:#0b0b10;color:#f4f4f5;font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Hiragino Sans GB",system-ui,sans-serif}main{min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;letter-spacing:.01em}.mark{display:grid;place-items:center;width:44px;height:44px;border-radius:10px;background:#f4f4f5;color:#0b0b10;font:800 15px ui-monospace,SFMono-Regular,Menlo,monospace}h1{margin:4px 0 0;font-size:18px;font-weight:650}p{margin:0;color:#a1a1aa;font-size:13px}span{width:96px;height:2px;overflow:hidden;border-radius:99px;background:#27272a}span::after{content:"";display:block;width:45%;height:100%;border-radius:inherit;background:#f4f4f5;animation:load 1.1s ease-in-out infinite}@keyframes load{from{transform:translateX(-120%)}to{transform:translateX(260%)}}</style>
</html>`)}`;
}
