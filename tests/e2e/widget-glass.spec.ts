...
test("7 日 Token 趋势悬浮显示对应日期，移出恢复今日摘要", async ({ page }) => {
  ...
  const firstBar = page.locator(".tt-glass-bar-column").first();
...
test("浮窗没有设置入口并始终保留三个 Agent 槽位", async ({ page }) => {
  ...
  await expect(page.locator('.tt-glass-overview a[href^="/"]')).toHaveCount(0);
  ...
  await expect(page.locator(".tt-glass-settings")).toHaveCount(0);
...
test("浮窗应用入口全部交给桌面主窗口路由", async ({ page }) => {
  ...
  await page.locator(".tt-glass-security").click();
...
test("Electron 浮窗模式完整绕过主应用壳且不裁切", async ({ page }) => {
  ...
  const stage = page.locator(".tt-widget-float-stage");
  await expect(stage).toHaveCSS("background-image", "none");
  await expect(stage).toHaveCSS("background-color", "rgba(0, 0, 0, 0)");

  const box = await widget.boundingBox();
  const securityBox = await page.locator(".tt-glass-security").boundingBox();
  const footerBox = await page.locator(".tt-glass-footer").boundingBox();
  ...
  await expect(page.getByRole("button", { name: "小组件设置" })).toHaveCount(0);
  await expect(page.locator(".tt-glass-watermark")).toHaveCount(0);
  expect(pageErrors).toEqual([]);
});
