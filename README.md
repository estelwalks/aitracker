...
采集范围覆盖 27 个产品目录工具，并额外支持 AiPy 和 Cline 遗留采集源。复杂来源（SQLite、累计快照、OTel 和多文件会话）由内置采集运行时处理；AiPy、Claude Code、Codex 和 WorkBuddy 同时保留 TrustTools 原生 reader 作为校验与降级路径。
