...
async function main() {
  ...
  const artifacts = [
    {
      arch: "x64",
      dmg: join(releaseDirectory, `TrustTools-${version}-x64.dmg`),
      app: join(releaseDirectory, "mac", "TrustTools.app"),
    },
    {
      arch: "arm64",
      dmg: join(releaseDirectory, `TrustTools-${version}-arm64.dmg`),
      app: join(releaseDirectory, "mac-arm64", "TrustTools.app"),
    },
  ];
