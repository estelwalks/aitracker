...
定价架构采用内置 seed、人工复核规则、来源专用名称归一化、精确匹配优先和未知模型诊断；未知模型绝不按零价格处理。
...
| 防止误匹配高价/低价模型 | 精确 > alias > 受限前后缀 > 通用 fallback；同优先级歧义构建失败。      | 不采用任意子串反查，可能留下更多待配置模型。             |
...
| 真实模型名样本覆盖                | 转换规则的准确性                 | M0 提取匿名样本和内部规则对照；未知项进入报告。          |
...
2. **P2：转换与默认策略。** 建立受限 matcher 和每工具 fallback；unknown 不再以零费用呈现。
...
- 将模型名称归一化与匹配实现为可审计 JSON 规则和固定解释器，禁止模糊子串匹配与未知零价行为。
