...
## ADR-009：TokenTracker 仅作为数据源行为参考
...
TokenTracker v0.83.6（提交 `32df4fe`）已经覆盖多种本地 AI 工具数据源，能够提供数据位置、格式和异常行为方面的研究价值。项目采用 MIT 许可证，但 TrustTools 后续计划独立开源，产品明确要求代码、接口和实现结构不能与其相同。
...
- TokenTracker 只提供行为研究输入，不提供实现模板。
